...
cdef class SampleValue(object):
    cdef public float value
    cdef public object channelMeta
...
cdef class Sample(object):
    ...
    cdef public object metas
    cdef public bint updated_meta
    
    def __init__(self, **kwargs):
        self.tick = kwargs.get('tick', 0)
        self.samples = kwargs.get('samples', [])
        default_metas = ChannelMetaCollection()
        self.metas = kwargs.get('channelMetas', default_metas)
        self.updated_meta = len(self.metas.channel_metas) > 0
        
    def fromJson(self, json):
        cdef object sample, metaJson, dataJson
        if json:
            sample = json.get('s')
            if sample:
    ...
    def processData(self, dataJson):
        ...
        cdef int maxFieldCount = channelConfigCount + bitmaskFieldCount
                
        cdef object fieldData = dataJson
        cdef int fieldDataSize = len(fieldData)
        ...
        cdef int bitmapIndex = 0
        cdef int fieldIndex = 0
